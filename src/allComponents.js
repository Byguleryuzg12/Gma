...
function HistoryModal({
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ticker: c.ticker, name: c.full,
          sector: c.sector, price: c.price, change: c.change
        })
      });

      // Fallback: direct Intelligence Layer call if serverless unavailable
      if (!res.ok && res.status !== 401) {
        const apiKey = localStorage.getItem('gma_platform_key') || '';
        if (apiKey) {
          const prompt = '"' + c.full + '" (' + c.ticker + ', $' + c.price.toFixed(2) + ', ' + c.change.toFixed(2) + '%) GMA Structured Analysis JSON: {"summary":"2-3 sentence position","positive":["f1","f2","f3"],"negative":["r1","r2","r3"],"innovation":["d1","d2"],"sentiment":"POZITIF|NOTR|NEGATIF","strengthScore":75,"riskScore":40}';
          const r2 = await fetch('https://api.anthropic.com/v1/messages', {
            method: 'POST',
            headers: {'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},
            body: JSON.stringify({model:'claude-sonnet-4-6',max_tokens:1000,system:'Return ONLY valid JSON. First character {.',messages:[{role:'user',content:prompt}]})
          });
          if (r2.ok) {
            const d2 = await r2.json();
            const txt2 = (d2.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('');
            const c2 = txt2.replace(/```json\s*/gi,'').replace(/```\s*/g,'').trim();
            const s2=c2.indexOf('{'), e2=c2.lastIndexOf('}');
            if (s2!==-1&&e2>s2) {
              const p2=JSON.parse(c2.slice(s2,e2+1));
              p2._creditsLeft=_gmaCredits(_user.email); p2._fallback=true;
              setAnalysis(p2); return;
            }
          }
        }
      ...
      const result = Object.assign({}, data.consensus, {
        ...
        _modelsUsed: data.modelsUsed || ['claude']
      });
      setAnalysis(result);
  ...
  /*#__PURE__*/React.createElement("div", {
  ...
  }, "GMA Sovereign Core v4 is fetching data..."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "13px",
      color: "#94a3b8",
      marginTop: "6px"
    }
  }, c.name, " market analysis is being prepared")), aiError && /*#__PURE__*/React.createElement("div", {
    style: {
      margin: "20px 0"
    }
  ...
  }, "SIGN IN / SIGN UP \u2192")) : aiError === '__NO_CREDITS__' ? /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center",
      padding: "32px 20px"
    }
    ...
    /*#__PURE__*/React.createElement("button", {
      onClick: () => { onClose(); window.location.href = "https://globalmarketanalytics.com/#pricing"; },
      style: {
        background: "linear-gradient(135deg,#f59e0b,#ef4444)",
        border: "none", color: "#fff", borderRadius: "10px",
        padding: "12px 28px", cursor: "pointer", fontSize: "14px",
        fontFamily: "inherit", fontWeight: "bold", letterSpacing: "0.06em"
      }
    }, "\uD83D\uDE80 Choose a Plan and Continue \u2192"),
    /*#__PURE__*/React.createElement("div", {style:{fontSize:"11px",color:"#64748b",marginTop:"10px"}},
 ...
 : /*#__PURE__*/React.createElement("div", {
  ...
  }, "\uD83E\uDD16 Start AI Analysis"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "13px",
      color: "#94a3b8",
      marginTop: "10px"
  ...
  }, sentimentIcon(analysis.sentiment)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "13px",
      color: sentimentColor(analysis.sentiment),
      fontWeight: "bold",
  ...
  }, analysis.sentiment)), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    text: analysis.kisaTimeframe,
    color: "#38bdf8"
  }, {
    label: "LONG TERM (1-3 years)",
    text: analysis.uzunTimeframe,
    color: "#a78bfa"
  }].map(({
    label,
    text,
  ...
  }, "\uD83D\uDE80 UPCOMING INNOVATIONS / DEVELOPMENTS"), analysis.innovation?.map((item, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      display: "flex",
      gap: "8px",
